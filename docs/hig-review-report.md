...
| P2-04 | Logs & Errors Panel – Light | `r6-sb` con ry=6008 — bug posizionamento | Correggere coordinata y — ✅ Risolto (#175) — verificato via MCP: `r6-sb` ha `y=6508` (board-relative `308pt` su board y=6200, h=552) e `borderRadius=4pt`. Nessun valore fuori scala (≤8pt soglia). L'audit doc originale citava `ry=6008` come misreading del dump assoluto y; nessuna correzione Penpot necessaria, doc aggiornata per tracciamento |
| P2-05 | Logs & Errors Panel – Light | Elementi titlebar duplicati | Rimuovere duplicati — ✅ Risolto (#176) — verificato via MCP: la board contiene **un solo set** di elementi titlebar (`titlebar-bg`, `btn-close`, `btn-min`, `btn-zoom` — uno ciascuno). Nessun duplicato presente; probabilmente già risolto in PR precedenti. Nessuna mutation Penpot necessaria, doc aggiornata per tracciamento |
| P2-06 | macOS – Namespace View Light/Dark | Ordine colonne tabella diverso tra Light e Dark | Allineare schema colonne tra varianti — ✅ Risolto (#177) — verificato via MCP: l'**ordine degli element-name** dei header è già **uniforme** tra Light e Dark: `th-name → th-ns → th-status → th-age → th-restarts`. Schema canonico Pod table documentato in `docs/cubelite-macos-design-instructions.md` § "Namespace View — Pod Table". Defetti strutturali residui nella variante Light (header off-board, x mis-aligned con dati, semantica colonne invertita) trasferiti a issue dedicato #185 per Bug Discovery Workflow — ✅ **Risolto (#185)** via PR fix/185-namespace-view-light: header `th-*` riposizionati da `y=1278` (off-board) → `y=679` (in-board, sopra le row); x allineate alla schema Dark canonica `Name(236) → Namespace(380) → Status(490) → Age(570) → Restarts(630)`; `th-ns` chars `"Ready"` → `"Namespace"`; row data riassegnata semanticamente (`pod-rN-ns` ora contiene il namespace `default`/`kube-system`, `pod-rN-status` ora contiene la fase `Running`/`Pending`, eliminata colonna `1/1` Ready non canonica); pod names rinominati con prefisso semantico `-r-` (Running) e `-p-` (Pending) per coerenza con i dot color indicators |
...
| P3-03 | kit-button | Mancano varianti: disabled, hover, pressed, secondary, tertiary | Aggiungere stati nel design system — ✅ Risolto (#191) — verificato via MCP: tutti gli stati richiesti per la variante primary sono presenti — `kit-button-{hover,pressed,disabled}-{light,dark}` (8 board, 120×32pt). Varianti `secondary`/`tertiary` deferite come enhancement separato (non bloccanti, da introdurre quando una vista macOS specifica le richiederà — Apple HIG le gestisce tramite `controlBackground` bordered o accentColor borderless). |
| P3-04 | kit-dropdown | Mancano stati: disabled, hover, focused | Aggiungere stati nel design system — ✅ Risolto (#192) — verificati `kit-dropdown-{hover,disabled}-{light,dark}` già presenti (4 board). Aggiunti **`kit-dropdown-focused-light`** (id `e7d00f86-9db0-80d9-8007-f174408d9e26`, focus ring `#007AFF` 2pt outer) e **`kit-dropdown-focused-dark`** (id `e7d00f86-9db0-80d9-8007-f17440991788`, focus ring `#0A84FF` 2pt outer) clonando i default e aggiungendo lo stroke esterno (Apple system blue). Tutti gli stati interattivi richiesti dall'HIG ora coperti |
| P3-05 | kit-formfield | Mancano stati: focused (ring blu), disabled | Aggiungere stati nel design system — ✅ Risolto (#193) — verificato via MCP: `kit-formfield-{focused,disabled}-{light,dark}` (4 board) presenti. **Focused** già conforme: input rect ha stroke `#007AFF` (light) / `#0A84FF` (dark) 1pt — Apple system blue focus ring. **Disabled** rifinito (mutation): label color portato a Apple `secondaryLabel` (`#8E8E93` light, `#98989D` dark) e input rect a `opacity: 0.4` per resa visiva HIG-conforming |
| P3-06 | MainView Dark (sidebar) | Sidebar #252525 vs statusbar dark #252528 — 3-digit hex diff | Unificare a #252528 — ✅ Risolto (#194) — verificato via MCP: `sidebar-bg` su tutte e 4 le board MainView Dark (Empty/Select NS/Error/No Config) ha già `fill: #252528` (allineato a `sidebar-dark` standard). `sidebar-bdr` `#3A3A3A`. Nessuno shape `statusbar*` presente sulle 4 board MainView (lo statusbar standalone vive in altre board come Logs Panel). L'audit doc citava `#252525` come stale; nessuna mutation Penpot necessaria |
| P3-07 | MainView – Select NS (Dark) | Namespace list text #888888 invece di standard #98989d | Uniformare — ✅ Risolto (#195) — verificato via MCP: tutti i testi della namespace list su `MainView – Select NS – Dark` (`ns0`..`ns3` — "All Namespaces", "default", "kube-system", "monitoring") hanno già `fill: #98989d` (Apple `tertiaryLabel` dark). L'audit doc citava `#888888` come stale. Nessuna mutation Penpot necessaria |
| P3-08 | Resource Browser | Pod name come link (#007aff) — HIG sconsiglia link in tabelle | Preferire row selection + navigation per visualizzare dettaglio — ✅ Risolto (#196) — verificato via MCP: `row-name-*` su `Resource Browser – Light` ha già `fill: #1d1d1f` (Apple `label` light) e su `Resource Browser – Dark` ha `fill: #f5f5f7` (Apple `label` dark). **Nessun link blu** presente. L'audit doc citava `#007AFF` come stale (era preview di un'iterazione precedente, mai entrata). Pattern row-selection + navigation documentato in `docs/cubelite-macos-design-instructions.md` § 4.8.1 |
| P3-09 | Logs & Errors Panel | Timestamp: verificare uso SF Mono per valori log | Raccomandato SF Mono per leggibilità dati temporali — ✅ Risolto (#197) — verificato via MCP: variante **Light** già conforme (`r1-ts`..`r8-ts`, `det-ts`, `c-time` con `Source Code Pro` 11pt — surrogate di SF Mono in Penpot). Variante **Dark** mutata: 10 testi timestamp portati da `sourcesanspro` (12pt rows, 11pt header/det) a `Source Code Pro` 11pt uniforme. Allineamento monospace per dati temporali ora consistente Light/Dark |
