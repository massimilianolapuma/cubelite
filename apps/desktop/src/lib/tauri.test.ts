...
import type { ContextInfo, PodInfo, NamespaceInfo, DeploymentInfo } from '$lib/tauri';
...
describe('listContexts', () => {
	it('invokes list_contexts and returns contexts', async () => {
		const mockContexts: ContextInfo[] = [
			{ name: 'minikube', cluster_server: 'https://127.0.0.1:8443', namespace: 'default', is_active: true },
			{ name: 'production', cluster_server: 'https://k8s.example.com', namespace: 'default', is_active: false }
		];
...
describe('setContext', () => {
	it('invokes set_context with context name', async () => {
		...
		expect(mockedInvoke).toHaveBeenCalledWith('set_context', { contextName: 'production' });
	});

	it('rejects when context does not exist', async () => {
		mockedInvoke.mockRejectedValueOnce(new Error("context 'nonexistent' not found"));

		await expect(setContext('nonexistent')).rejects.toThrow("context 'nonexistent' not found");
	});
});
...
describe('listPods', () => {
	...
	it('handles pods with null phase', async () => {
		const mockPods: PodInfo[] = [
			{ name: 'unknown-pod', namespace: 'default', phase: null, ready: false, restarts: 3 }
		];
		mockedInvoke.mockResolvedValueOnce(mockPods);
...
describe('watchResources', () => {
	it('invokes watch_resources and returns watch ID', async () => {
		...
		const result = await watchResources('/kube/config', 'pods', 'default', 'minikube');
...
describe('unwatchResources', () => {
	it('invokes unwatch_resources with watch ID', async () => {
		...
		expect(mockedInvoke).toHaveBeenCalledWith('unwatch_resources', { watchId: 'watch-abc-123' });
	});
});
